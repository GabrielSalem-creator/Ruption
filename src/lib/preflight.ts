...
export function runPreflight(url: string): PreflightResult {
  ...
  const looksInternal = normalizedUrl.includes("/runtime/") || normalizedUrl.includes("vercel.app") || normalizedUrl.includes("localhost");
  ...
  return {
    ...
    iframeCompatible: !normalizedUrl.includes("login") && !normalizedUrl.includes("auth"),
